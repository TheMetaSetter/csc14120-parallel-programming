		...
		uint8_t * outPixels)
{
	...
	int redIdx = globalThreadIdx * 3;
	int greenIdx = redIdx + 1;
	int blueIdx = greenIdx + 1;

	if (globalThreadIdx < width * height) {
		int grayValue = 0.299 * inPixels[redIdx] + 0.587 * inPixels[greenIdx] + 0.114 * inPixels[blueIdx];
		outPixels[globalThreadIdx] = (uint8_t)__float2uint_rn(fminf(fmaxf(grayValue, 0.0f), 255.0f));
	}
}

void convertRgb2GrayOnHost(uint8_t * inPixels, int width, int height, uint8_t * outPixels) {
	for (int i = 0; i < width * height; ++i) {
		int redIdx = i * 3;
		int greenIdx = redIdx + 1;
		int blueIdx = greenIdx + 1;

		if (blueIdx >= width * height * 3) continue;

		int grayValue = 0.299 * inPixels[redIdx] + 0.587 * inPixels[greenIdx] + 0.114 * inPixels[blueIdx];
		outPixels[i] = (uint8_t)lrintf(fminf(fmaxf(grayValue, 0.0f), 255.0f));
	}
}
		...
		bool useDevice=false, dim3 blockSize=dim3(1))
{
	...
	if (useDevice == false)
	{
		// TODO
        // Reminder: gray = 0.299*red + 0.587*green + 0.114*blue  
		convertRgb2GrayOnHost(inPixels, width, height, outPixels);
	}
	else // use device
	{
		...
		size_t inPixelsNBytes = width * height * 3 * sizeof(uint8_t);
		size_t outPixelsNBytes = width* height * sizeof(uint8_t);
		CHECK(cudaMalloc(&d_inPixels, inPixelsNBytes));
		CHECK(cudaMalloc(&d_outPixels, outPixelsNBytes));
		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(d_inPixels, inPixels, inPixelsNBytes, cudaMemcpyHostToDevice));
		CHECK(cudaMemcpy(d_outPixels, outPixels, outPixelsNBytes, cudaMemcpyHostToDevice));
		// TODO: Set grid size and call kernel (remember to check kernel error)
		dim3 gridSize((width*height - 1) / blockSize.x + 1);
		convertRgb2GrayKernel<<<gridSize, blockSize>>>(d_inPixels, width, height, d_outPixels);
		CHECK(cudaGetLastError());
		cudaDeviceSynchronize();
		// TODO: Copy result from device memories
		CHECK(cudaMemcpy(outPixels, d_outPixels, outPixelsNBytes, cudaMemcpyDeviceToHost));
